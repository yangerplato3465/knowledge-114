...
- 作答必須攜帶 questionId；同題重複作答與舊題回呼拒絕處理。已逾期限的點擊視為超時，不依賴瀏覽器計時器先後順序。
- 重玩、下一題及離場取消舊計時器並遞增世代，即使被取消回呼仍送達也不會更新新局。離場不可重啟，通知拋錯時停止背景排程。
- 新增 8 項測試：時間線、超時、重玩、失效回呼、六關選卡／勝利、敗北、快照隔離與通知例外；21 項 Node + 65 項 Vitest（86 項）、typecheck、production build 及舊站相容檢查通過。
- 下一批接入 React 題目／HUD 與 renderer；元件 StrictMode、實際 canvas 清理及瀏覽器操作仍待驗收。本批未提交、推送或部署。
