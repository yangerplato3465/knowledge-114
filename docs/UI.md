...
## 素材下載／上傳 React（2026-09-17）

素材下載／上傳 React 頁（2026-09-17）使用共用 PageLayout 與原主題 token，檔案列長名稱可換行、操作目標至少 44px。載入與空清單使用 status、錯誤使用 alert；上傳逐檔提供文字進度，保留原生刪除確認。保留可鍵盤使用的原生檔案選擇器，拖放為附加入口。

## 酸鹼教材 React（2026-09-14）

保留明亮投影片與 SVG 圖解的既有深色策略。投放及答题使用原生按鈕、至少 44px 目標與可見焦點，文字 status 補充溫度、離子與變色。1320px 以下採流動排版，reduced-motion 停用裝飾動畫，實驗結果仍按原投放時間更新。
