...
#
# 2026-08-27 四版起改成**逐張量地面線**再回填，不再沿用舊值：
#
#   cropY = 地面線在原圖的 y - OFFSET      OFFSET 目前是 212
#
#   OFFSET 不是「腳底% x 269」那麼單純，因為容器看到的不是整條帶：
#   cover 縮放比 max(1254.8/1600, 298.2/420) = 0.784 由**寬度**決定，
#   帶被算成 1254.8x329.4，而容器只有 298.2 高，又是 background-position:center bottom，
#   所以帶頂被切掉 31.2 個螢幕 px（= 帶內 39.8px = 裁切帶內 25.5px）。
#
#   推導（1280x720、u=14.4 實測）：
#     容器上緣 = 帶內 y 39.8       容器高 298.2 = 帶內 380.2px
#     腳底 76.56%（瀏覽器實測，不是註解裡的舊值 71%）
#       -> 帶內 y = 39.8 + 0.7656 x 380.2 = 330.9  -> /420 x 269 = 212
#
#   ⚠️ 腳底% 由 pages/math-rpg.html 的 .sprite{bottom} + .sprite-slot{margin-bottom}
#      + .fighter .name{line-height,margin} 共同決定。動到那幾個值就要重量腳底%、
#      重算 OFFSET、六張全部重跑。2026-08-27 為了縮小角色與名字的空隙就動過一次
#      （OFFSET 200 -> 212，六張 cropY 全部 -12）。
#   量法（把 PNG 轉 PPM 掃綠通道落差）見 assets/images/math-rpg/STAGES.md 第五節。
#
#   stage1 是新版草原：地面線量到 y=376 (65.7%) -> cropY = 376-212 = 164。
#   stage2~6 仍是 2026-08-20 的舊圖，值是從舊版各 -12 平移來的；換新圖時各自重量。
JOBS=(
  "1:164:"
  "2:165:"   # 新版密林，實測地面線 y 377 (65.9%)
  "3:176:"   # 新版洞窟，實測地面線 y 388 (67.8%)；原圖已是青綠，不再需要 hue 轉色
  "4:190:"   # 新版腐沼，岸緣是波浪狀沒有單一線，靠疊圖比對定在 190（偵測器給的 183 會浮在水面上）
  "5:137:"   # 新版熔岩，實測地面線 y 349 (61.0%)；原圖有上下黑邊，可見帶剛好閃過
  "6:219:"   # 新版王座廳，實測地板線 y 431 (75.3%)。⚠️ 不是 373 —— 那是牆上的裝飾金帶
)
