export interface Question { q: string; a: string[]; correct: number }
export type QuestionPool = Question[] | (() => Question);
export type QuestionPools = Record<string, Record<string, QuestionPool>>;

/** 每個实例注入自己的亂數，保留舊題庫與產生器的抽樣順序。 */
export function createQuestionPools(random = Math.random): QuestionPools {
// === 各題庫的佔位用題目 (placeholder questions per pool) ===
// 這個檔案只放題庫資料，方便日後新增 / 修改題目。
// 由舊 math-rpg-pools.js 等價移植；每個 factory 實例保有自己的亂數來源。

// === 整數、小數除以整數：動態產生題目（不使用固定題庫）===
// 規則：答案最多 3 位小數、兩數皆 2 位數以內、適合五年級、選項夠接近不會太明顯
function poolRandInt(min: number, max: number) { return Math.floor(random() * (max - min + 1)) + min; }
function poolRound3(x: number) { return Math.round(x * 1000) / 1000; }
function poolTerminates3(x: number) { return Math.abs(x * 1000 - Math.round(x * 1000)) < 1e-9; }
function poolDecimals(x: number) {
    const s = poolRound3(x).toString();
    const i = s.indexOf('.');
    return i === -1 ? 0 : s.length - i - 1;
}
function poolFmt(x: number) { return parseFloat(poolRound3(x).toFixed(3)).toString(); }
function poolShuffle<T>(arr: T[]) {
...
function generateDivideQuestion(): Question {
    ...
    const asFraction = random() < 0.5; // 兩種題型：分數化小數 或 直接除法
    const q = asFraction
        ? `${a}/${b} = ?（用小數表示）`
        : `${a} ÷ ${b} = ?（用小數表示）`;
...
const POOL_FRACTION_DIV: Question[] = [
...
const POOL_DECIMAL_DIV: Question[] = [
...
const POOL_RATIO_VALUE: Question[] = [
...
const QUESTION_POOLS: QuestionPools = {
    "五年級": POOLS_G5,
    "六年級": POOLS_G6
};
...
export const questionPools = createQuestionPools();
