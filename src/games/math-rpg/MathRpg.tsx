    ...
    <div className="mr-panel">
      {state.paused ? <><h2 ref={panel} tabIndex={-1}>先休息一下</h2><p>戰鬥與自動換題已暫停。準備好再繼續。</p>{state.phase === 'feedback' && <div><strong><FractionText text={question.q} /></strong><p><FractionText text={`正確答案：${question.a[question.correct]}。${question.hint}`} /></p></div>}<div className="mr-actions"><button className="mr-primary" onClick={() => dispatch({ type: 'resume' })}>返回戰鬥</button><button onClick={onLeave}>結束本局，回到準備</button></div></>
        : state.phase === 'battle' || state.phase === 'feedback' ? <>
          ...
          <div className="mr-actions">{state.phase === 'lost' && <button className="mr-primary" onClick={() => { const questionScale = nextQuestion(); dispatch({ type: 'retry', questionScale }); }}>回滿 HP，再戰本關</button>}<button onClick={onLeave}>回到準備</button></div>
        </>}
    </div>
...
export function MathRpg() {
  ...
  return <PageLayout activityTitle="數學勇者">{session ? <Adventure key={session.seed} session={session} onLeave={() => setSession(null)} /> : <section className="mr-shell mr-setup">
    ...
    <form onSubmit={event => { event.preventDefault(); if (ready) setSession({ grade, unit, seed: crypto.getRandomValues(new Uint32Array(1))[0] }); }}>
      ...
      <p className="mr-note" role="status">{ready ? <>{unit === GEOMETRY_UNIT ? '五上第 5 單元：三角形邊長、三／四邊形內角、平角及扇形角度與全圓分數。四選一；多步推算題提供較長蓄力時間。' : unit === DECIMAL_UNIT ? '五上第九冊第一單元：隨機練習小數加減、四捨五入與比大小。' : unit === FRACTION_UNIT ? '五上第九冊第 4 單元：除法轉分數、帶分數、通分比較及擴分約分。觀察題只練習分子都是 1、或分母比分子多 1；比大小三選一，其餘四選一。此單元採中間戰鬥節奏，蓄力比小數短、比因數倍數長。' : '五上第九冊第 2–3 單元：因數與倍數合併練習，包含公因數、最大公因數、公倍數、最小公倍數及 2、5、10 的倍數判斷。此單元採較快戰鬥節奏，五關怪物的蓄力時間較短。'}可準備紙筆計算，需要討論時可暫停；紙筆作答的遊戲時長仍待教室試玩校準。角色與敵人目前仍為佔位美術。</> : '此單元題庫尚未開放。目前可選擇五上的「多位小數與加減」、「因數與倍數」、「擴分、約分與通分」或「多邊形與扇形」開始冒險。'}</p><button className="mr-primary" type="submit" disabled={!ready}>{ready ? '開始五關冒險' : '題庫尚未開放'}</button>
    </form>
  </section>}</PageLayout>;
}
